 ...
 * Props: title, description, price (optional)
 */
export default function ServiceCard({ title, description, price }) {
    return (
        <div className="card flex flex-col h-full hover:shadow-md transition-shadow duration-200">
            {/* Icon accent */}
            <div className="w-10 h-10 rounded-lg bg-primary-50 border border-primary-100 flex items-center justify-center mb-4 shrink-0">
            ...
            {price != null && (
                <div className="mt-4 pt-4 border-t border-gray-100">
                    <span className="text-primary-600 font-semibold text-sm">
                        Starting at &#8377;{Number(price).toLocaleString('en-IN')}
                    </span>
                </div>
            )}
        </div>
